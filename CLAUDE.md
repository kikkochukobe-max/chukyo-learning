...
生徒の学習記録機能を追加するプロジェクト。設計詳細は `schema_full.sql`（16テーブル・検証済み）と
`mypage_mock.html`（マイページ確定デザイン）を参照。詳細な引き継ぎ経緯は `HANDOFF.md` にも記載。
...
5b. **講師確認ページ → 完了（/teacher.php）**: 講師ログインフォーム内蔵。
   生徒一覧（期間タブ+教室タブ、学習時間/解答数/正答率/解き直し数/最終学習）→
   生徒名クリックで詳細（単元カルテ・直近の誤答30件・学習セッション20件+端末）。
   権限: super_admin=全教室 / それ以外=teacher_classroomsの担当教室のみ。
...
`mypage_mock.html` が生徒マイページの確定デザイン。
