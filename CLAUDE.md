...
5d. **アカウント管理ページ → 完了（/admin.php）**: 生徒登録/生徒一括登録(Excel貼り付け・CSV)/
   保護者登録/講師登録(統括のみ)/登録一覧 をタブで切替。登録一覧は開くたび再取得、
...
PINは生徒と同じ4桁数字。同じ代表の子で二重登録すると login_id 衝突で409（＝その家庭は登録済み）。
保護者テーブルは別なので生徒コードと文字列衝突しない（`g`接頭辞で数字の生徒コードとも明確に区別）。保護者専用ログインは後日リリース。
