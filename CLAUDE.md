...
assets/                共通モジュール（全ツールがscriptタグで読み込む）
  ...
  divp-correct-kirakira.js  小学生用 正解エフェクト「キラキラが積もる」。
                         ...
                         ⚠ **画面の下に固定した帯があるツールは `pileBottom` を渡すこと**。
                         山は1回の正解で「画面の高さ÷pileFull」しか育たない
                         （900pxの画面で9px）ので、不透明な帯（「こたえあわせ」の
                         ボタン列など74px）が積もりはじめを丸ごと隠し、9問正解するまで
                         **「積もらない」ように見える**（実際にそう見えた）。
                         `Divp.correct({pileBottom:"#bottom"})` のようにセレクタを渡すと
                         その帯の高さを毎回測って床にする（数値pxでも可。
                         隠れている帯は0とみなす＝モード選択画面でも破綻しない）。
...
2a. **question_text の数式マーカーと、それを描く3つのレンダラー**。
   ...
   **分数のつもりの `/` は必ず `F()` で包む**（question_params 側は params_hash に効くので書式を変えない）。
   ⚠ **`SYS()` は question_text の全体が SYS(…) のときしか効かない**（PHP側が `/^SYS\(…\)$/` で見る）。
   「連立方程式 SYS(…) の解が…」のように地の文に埋めても展開されない。そのうえ
   **question_text に `{` `}` `\` `^` `_` が1文字でもあると文字列ぜんぶをTeXとして描く**ので、
   和文がmathモードに入って斜体で崩れる。地の文に式を入れたい
   ときは中かっこを使わず「ax-by=-1、bx-ay=13」のように 「」 で囲んだ素のテキストで書くこと
   （連立の利用の `moji1`/`moji2` がこの形。画面はKaTeXで中かっこ付きに描き、記録用の1行だけ
   別に持つ＝ツール側の `q.plain` を `Divp.answer` の question_text に渡している）。
...
2b. **図が無いと解けない問題は question_figure で図そのものを保存する**。
   ...
   **誤答のときだけ** answer_logs.question_figure に入り、teacher.php の解き直し
   ...
   math_js3_heihokon の利用モード（かざりの図 `prob.fig`）/
   math_js2_ichijikansu_riyou（`表`・動く点の`長方形`・`座標平面`・`ダイヤグラム`
   すべて `q.fig`。グラフは目もりの数字を読んで解く図なので、紙で小さいと解けない＝
   svg に `class="divp-graph"` を付けて印刷CSSだけ大きさを変えている
   （色・線は属性が持つので、クラスが効かない解き直しプリントでも図は成立する））。
...
2f. **選択式の `question_choices` に生の `<` を書かない**。
   「問題文＋図だけでは紙で解き直せない」型のツールは
   `Divp.answer(ok,{… question_choices:[{t:'tex'|'svg', v:'…'},…]})` で選択肢も送るが、
   api/save_answer.php は `t:'tex'` の値に `<` が**1文字でもあると選択肢を丸ごと捨てる**
   （長さと「生の < が無いこと」だけで弾く作り。記録自体は普通に残るので気づきにくい）。
   不等号を選択肢に出すツール（「2点で交わる k の範囲」など）は
   TeX の `\lt` / `\gt` で書くこと（KaTeX はそのまま不等号に描き、講師画面も
   バックスラッシュを見て KaTeX に回すので見た目は同じ）。
   ⚠ **和文を含む選択肢に TeX を混ぜない**（「中心 (2, -1)、半径 2√5」「外接する」など）。
   `\` や `{}` が1つでもあると講師画面が文字列ぜんぶを TeX として描いて和文が崩れる。
   和文の選択肢は 2乗=「²」・ルート=「√」の素の文字で書き、**画面側も KaTeX に通さない**
   （円と方程式マスターは `ans.plain=true` で出し分けている）。
   回帰テストは tests/en-houteishiki.spec.js の「選択肢に生の「<」が入らない」。
...
8. XPの不正対策方針: 正解のみ付与、同一question_keyの大量連打は減衰、1日上限
   （実装済み: save_answer.php の XP_DECAY_RATE=0.7 と DAILY_XP_CAP=300）
8a. **不正対策の構え: 「あり得ない値は弾く／グレーは講師に印」の2段**。
   正誤判定はツール側（クライアント）の責務なので、**画面の答えを見て解く不正は
   サーバーでは原理的に防げない**。防げるのは「値そのものがあり得ない」ものだけで、
   そこは必ずサーバーで止める（クライアントのガードは詐称できるので歯止めにならない）。
   - 弾く: タイムの min_ms（5a2）/ miss_count が満点を超える / time_taken_sec の範囲外。
     拒否したものは error_log に生徒IDごと残す。
   - 印を出す: suspect_ms より速い記録に teacher.php で「速すぎ」バッジ（`.t-susp`）。
     **生徒のマイページには出さない**＝疑いを本人に突きつけない。先生が見て判断する。
   - **クライアントが時刻そのものを送る経路は作らない**。日付・時刻はすべて
     サーバーの NOW()／CURDATE()（自習記録の study_date も入力欄の上下限を
     PHP が埋め、保存時にサーバー日付で再判定する）。経過時間の計測は
     `performance.now()`（5a3）。
   - **解答数・正答率ランキングの数え方は変えていない**（同じ問題の解き直しも
     1件ずつ数える）。連打で数を稼ぐ余地は残るが、記録の素直さを優先した判断
     （2026-09）。変えるなら params_hash で重複を除くか1日の上限を入れる。
...
5a. **学習時間は活動ベースの積算方式**: 壁時計(開始〜終了)ではなく、
   ...
   ⚠ **加算処理は helpers.php の `touch_session_activity()` 1本**（解答・ハートビート・
   タイム保存・終了の4か所が呼ぶ。以前は同じSQLが4本に写っていた）。
   起点は「そのセッションの前回活動」ではなく**その生徒の前回活動**
   （同じ生徒の他セッションの最終活動より後のぶんしか足さない）。
   セッションごとに独立で数えると、**ツールを2窓で開いて放置するだけで
   学習時間が2倍になる**＝水増しできてしまう（悪意が無くても二重計上した）。
   何タブ開いても合計は実時間を超えない代わりに、**単元別の時間は
   最後に活動した方に寄る**（同時に2ツールを開いた場合）
5a2. **「1回のプレイにかかった時間」は time_records に1プレイ=1行**（学習時間 duration_sec とは別物）。
   ...
   **見せる単元の台帳は api/time_ranking.php の `time_units()`**（ここに無い unit_key は
   ...
   `min_ms`(これより速いタイムは save_time.php が**保存しない**) /
   `suspect_ms`(これより速いと講師ページに「速すぎ」の印。保存はする)。
   ...
   サーバー側も 0以下・1時間超は null にして保存する＝列が signed INT なので
   弾かないとマイナスがそのまま入る）
5a3. **タイムの計測に `Date.now()` を使わない**（`performance.now()` で測る）。
   `Date.now()` は端末の時計なので、**プレイの途中で時計を巻き戻すとタイムが縮む**
   ＝速さランキングを不正に取れる。`performance.now()` はページを開いてからの
   経過msなので端末の日付・時刻を変えても動かない。各ツールに
   `nowMs()`（performance.now が無い環境向けのフォールバック付き）を1つ置いて、
   経過時間はすべてそれで測る。**「いつ解いたか」はサーバーの NOW() が持つ**ので、
   ツール側が時刻そのものを送る経路は作らない。
   直した2本: math_es_hyakumasu / math_js3_aichi_daimon1（saveTime を呼ぶツール）。
   ⚠ クライアントの計測値はいくらでも詐称できるので、これは「うっかり・手軽な不正」の
   封じ込め。**最後の歯止めは台帳の min_ms によるサーバー側の拒否**（5a2）。
   ⚠ `time_taken_sec` を送るだけのツールは `(sec>0 && sec<600)` のガードを持つ
   （どこにも表示しない値なので、詐称しても得るものが無い）
...
5g. **自習の記録 → 完了（self_study_logs）**: 生徒が「家で何を自習したか」を自分で書き残し、
   ...
   **送信された時刻も残る**（self_study_logs.created_at＝テーブル作成時からある列なので
   SQLの追加実行は不要。DBセッションが JST なのでそのまま日本時間）。表示は
   self_study_sent_label() が作り、自習した日と同じ日に送っていれば「21:34 送信」、
   あとから書いたなら「9/18 7:02 送信」と日付付きで出す＝まとめ書きが見分けられる
   （teacher.php の自習報告ビュー・生徒詳細 / mypage.php / guardian.php の3画面に出る）。
   ⚠ **なおした時刻は出さない**。updated_at は講師が確認印を押しただけでも動くので
   「生徒がなおした時刻」としては読めない（出したくなったら専用の列を足すこと）。
   ...
   **保護者ページ(guardian.php)にも表示済み**: お子さまごとのカード内、単元カルテの下に
   「自習の記録」を出す（読むだけ＝書く・なおす・けす・確認印は出さない。
   講師の個人名も出さずマイページと同じく「講師から」表記）。
   ⚠ ここは **API(list_self_study.php)を使わずPHPで直接SELECTしている**
   （guardian.php は全部サーバー描画なので、この1枚だけJSにすると描画が2本になる。
   API側は guardian でも読める状態のまま＝別画面から使うなら再利用できる）。
   ⚠ 期間タブぶんの記録に加え、足あとの日付タップ用に直近35日ぶんも
   `hidden` で描いておき、JS は表示の出し分けだけをする（`data-date` /
   `data-inperiod` / `data-min`）。**HTMLの組み立てはPHP側の1本きり**にしてある。
   ⚠ migrate_self_study.sql 未適用でもページが落ちないよう try/catch で囲んである
