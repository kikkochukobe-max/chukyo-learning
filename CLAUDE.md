...
生徒の学習記録機能を追加するプロジェクト。設計詳細は `schema_full.sql`（16テーブル・検証済み）を参照。
マイページのデザインは実装済みの `mypage.php` が正（元モックの mypage_mock.html は実装完了に伴い削除済み）。
詳細な引き継ぎ経緯は `HANDOFF.md` にも記載。
...
| 保護者 | guardians / guardian_students | 兄弟対応の多対多。**専用ログイン・保護者ページ(/guardian.php)実装済み**。パスワードは講師と同方式(仮8字英数→must_change_password=1→初回ログインで本人が8〜15字英数に変更。変更APIは api/change_password.php を講師と共用) |
...
5d. **アカウント管理ページ → 完了（/admin.php）**: 生徒登録/生徒一括登録(Excel貼り付け・CSV)/
   ...
   **生徒登録（単独・一括とも）で保護者アカウントを同一トランザクションで自動発行**
   （ID=g+生徒コード、仮パスワード8字英数を自動発行、表示名=生徒名+保護者様。
   register_student.php が guardian_login_id/guardian_password を返す）。
   登録後にご家庭向け案内文（生徒コード/PIN・保護者ID/仮パスワード+URL入り）を
   生成・コピーできる（単独=自動表示、一括=「案内文をまとめて生成」
   ボタン。保護者の仮パスワードはその画面でしか確認できない）。
   保護者は氏名もパスワードも入力しない（表示名・仮パスワードとも自動。
...
5e. **講師パスワード → 完了**: password.php + api/change_password.php（現PW照合必須。
   保護者の変更にも共用: actor_type で teachers/guardians を切替。保護者のUIは guardian.php 内蔵。
   guardians.must_change_password は migrate_guardians_must_change.sql で追加＝本番適用要）。
   ...
   保護者がPWを忘れた場合も登録一覧の保護者行「PW初期化」で仮PW再発行
   （api/reset_guardian_password.php、super_admin=全保護者 / classroom_admin=担当教室の
   生徒に紐づく保護者のみ＝教室で申し出を受けて教室で再発行できる）
...
パスワードは講師と同方式（仮パスワード8字英数を自動発行→本人が /guardian.php の初回ログインで
8〜15字英数に変更。生徒のPIN 4桁とは別物）。同じ代表の子で二重登録すると login_id 衝突で409（＝その家庭は登録済み）。
