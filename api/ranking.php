...
function ranking_rows(PDO $pdo, ?array $classroomIds, ?string $fromStr, ?string $toStr, bool $includeTest = false, ?string $unitKey = null, ?string $grade = null, ?int $targetSchoolId = null): array
{
    ...
    // 志望校フィルタ（私立・公立どちらかに設定していればヒット）
    $wSchool = '';
    if ($targetSchoolId !== null && $targetSchoolId > 0) {
        $params['tsid'] = $targetSchoolId;
        $wSchool = ' AND (s.target_private_id = :tsid OR s.target_public_id = :tsid)';
    }
    ...
    $stmt = $pdo->prepare(
        "SELECT s.student_id, s.student_name, s.grade, s.classroom_id, c.classroom_name,
         ...
         WHERE s.is_active = 1{$wc}{$wTest}{$wGrade}{$wSchool}"
    );
    $stmt->execute($params);
