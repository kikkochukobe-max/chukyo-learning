 ...
 */
function ranking_rows(PDO $pdo, ?array $classroomIds, ?string $fromStr, ?string $toStr, bool $includeTest = false, ?string $unitKey = null, ?string $grade = null): array
{
    $wc = '';
    ...
    if ($unitKey !== null && $unitKey !== '') {
        $params += ['unitA1' => $unitKey, 'unitA2' => $unitKey, 'unitX' => $unitKey];
        $wu1 = ' AND al.unit_key = :unitA1';
        $wu2 = ' AND al.unit_key = :unitA2';
    ...
    $stmt = $pdo->prepare(
        "SELECT s.student_id, s.student_name, s.grade, s.classroom_id, c.classroom_name,
                (SELECT COUNT(*) FROM answer_logs al
                  WHERE al.student_id = s.student_id{$wa1}{$wu1}) AS solved,
                (SELECT COALESCE(SUM(al.is_correct),0) FROM answer_logs al
                  WHERE al.student_id = s.student_id{$wa2}{$wu2}) AS correct,
                (SELECT COALESCE(SUM(xl.amount),0) FROM {$xpFrom}
                  WHERE xl.student_id = s.student_id{$wx}{$wux}) AS xp
         FROM students s
         JOIN classrooms c ON c.classroom_id = s.classroom_id
         WHERE s.is_active = 1{$wc}{$wTest}{$wGrade}"
    );
    $stmt->execute($params);
